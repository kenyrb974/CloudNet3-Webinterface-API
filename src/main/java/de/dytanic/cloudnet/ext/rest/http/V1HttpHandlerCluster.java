...
public final class V1HttpHandlerCluster extends HttpHandler {
    ...
    public void handleGet(String path, IHttpContext context) {
        if (context.request().pathParameters().containsKey("node")) {
            ...
            if (super.getCloudNet().getClusterNodeServerProvider().getNodeServers().size() != 0) {
                context
                        .response()
                        .statusCode(HttpResponseCode.HTTP_OK)
                        .header("Content-Type", "application/json")
                ...
                context
                        ...
                        .header("Content-Type", "application/json")
                        .body(GSON.toJson(
                                Arrays.asList( new JsonDocument()
                                        .append("node", getCloudNet().getClusterNodeServerProvider().getSelfNode().getNodeInfo())
                                        .append("nodeInfoSnapshot", getCloudNet().getCurrentNetworkClusterNodeInfoSnapshot()))
                        ))
                        .context()
                        .closeAfter(true)
                        .cancelNext();
