...
import de.dytanic.cloudnet.ext.rest.HttpSession;
import de.dytanic.cloudnet.http.V1HttpHandler;

import java.util.Map;

public final class V1HttpHandlerAuthentication extends HttpHandler {
  ...
  public void handle(String path, IHttpContext context) throws Exception {
    ...
    Map map = HTTP_SESSION.auth(context);
      ...
      if (context.request().queryParameters().containsKey("redirect")) {
        ...
        try {
          context
                  ...
                  .body(new JsonDocument("success", true).append("userUniqueId", HTTP_SESSION.getUser(context).getUniqueId()).append("cookie", map.get("cookie"))
                          .toByteArray())
                  .context()
                  .closeAfter(true)
                  .cancelNext();
